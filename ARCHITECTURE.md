...
当前实现已进入第三阶段：核心边界、Simulator 驱动、异步数据管道、Avalonia 工作台、正式 DI 组合根、Serilog 文件日志与 UI Sink、JSON 配置加载、SQLite 数据查看器适配器均已落地。
...
当前 Tag 改动保存后需要重启采集/应用后进入运行时扫描集合。后续可继续实现配置热加载和 Runtime 动态重建扫描任务。
...
2. 组合根装配 Simulator 驱动、连接池、Tag 管道、LiveLog 服务、Serilog、配置存储、数据查看器与 `HostRuntime`。
3. 启动时加载 `Config/hostsettings.json`，不存在时自动生成默认配置。
4. 点击 `Start` 后，`HostRuntime` 按设备分组启动扫描任务与心跳任务。
5. Simulator 驱动生成 Tag 值并附带质量戳、时间戳。
6. Tag 值进入有界管道并被批量发布。
...
1. 实现 Tag 配置热加载：保存后自动重建扫描任务并刷新主界面 Tag 列表。
2. 实现 DataGrid 版通用查看器：动态列、过滤条件、页码输入、Excel 导出。
3. 扩展 SQLite：配方、报警历史、操作审计、系统配置表。
4. 实现真实通信适配器：Modbus TCP/RTU、S7、FINS、SerialPort、USB HID。
5. 实现权限系统：登录、角色管理、工作区路由、控件级权限附加属性。
6. 引入 `Microsoft.Extensions.Hosting`，使 Runtime 生命周期与桌面应用生命周期更自然地绑定。
7. 增加自动化测试：Simulator 驱动、配置验证、管道背压、扫描调度。
